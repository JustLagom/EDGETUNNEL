...
async function getVLESSConfig(userID, hostName, sub, userAgent) {
	if (!sub) {
	  const vlessMain = `vless://${userID}@${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=random&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#${hostName}`;
	  return `
  <p>############################配置详解################################</p>
  <p>CF-pages-vless+ws+tls节点，分享链接如下:${pvlesswstls}</p>
  <p>##################################################################</p>
  `;
